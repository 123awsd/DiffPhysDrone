...
for i in pbar:
    ...
    for t in range(args.timesteps):  # 时间步循环
        ...
        target_v_unit = target_v_raw / target_v_norm#单位化保留方向
        ...
        x = F.max_pool2d(x[:, None], 4, 4)#下采样4倍 Bx1x12x16
        ...
        v_net_feats.append(torch.cat([act, local_v, h], -1))#拼接动作、局部速度、GRU隐藏状态
    ...
    target_v_history = torch.stack(target_v_history)#目标速度序列
    T, B, _ = v_history.shape#时间步数，批量大小，维度
    delta_v = torch.norm(v_history_avg - target_v_history[1:1-30], 2, -1)#计算30步平均速度与目标速度的差异
    loss_v = F.smooth_l1_loss(delta_v, torch.zeros_like(delta_v))  # 速度匹配损失

    v_preds = torch.stack(v_preds)#速度预测序列
    loss_v_pred = F.mse_loss(v_preds, v_history.detach())  # 速度预测损失

    target_v_history_norm = torch.norm(target_v_history, 2, -1)#目标速度模
    target_v_history_normalized = target_v_history / target_v_history_norm[..., None]#目标速度单位化
    fwd_v = torch.sum(v_history * target_v_history_normalized, -1)#计算速度与目标速度的点积，可以量化当前速度在目标速度方向上的投影大小
    ...
    vec_to_pt_history = torch.stack(vec_to_pt_history)#障碍物向量序列
    distance = torch.norm(vec_to_pt_history, 2, -1)#障碍物向量模
    distance = distance - env.margin#障碍物向量模减去安全半径
    with torch.no_grad():#计算接近速度
    ...
    pbar.set_description_str(f'loss: {loss:.3f}')#设置进度条描述
    ...
    with torch.no_grad():
        ...
        success = torch.all(distance.flatten(0, 1) > 0, 0)#判断是否成功避障
        ...
        smooth_dict({
            ...
            'ar': (success * avg_speed).mean()})#成功率和平均速度等指标
        log_dict = {}
        if is_save_iter(i):#用于判断是否需要在当前迭代步保存数据
            # 保存可视化图表
